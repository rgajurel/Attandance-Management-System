...
namespace SchoolManagementSystem.Areas.Client.Controllers
{
    public class MyQuizController : Controller
    {
        ...
        int QuizPerpage = 10;
        public MyQuizController(IQuizRepository QuizRepo, IQuizClientRepository QuizClientRepo)//, IStatusRepository statusRepo)
        {
       ...
       // [AuthorizeUser(Controls = "View")]
        ...
        //}
       private void GetQuizProgressForCurrentUser()
        {
        ...
        private void LoadQuizListinginPageLoad()
        {
            ...
            IEnumerable<QuizClientSide> QuizLst = _QuizClientRepo.GetQuizListingForMyQuiz(objInfo);
            ...
            else
            {
                ...
                {
                    PercentageValue = Math.Ceiling(((float)item.TotalQuizAnswered / (float)item.TotalQuestionInQuiz) * 100.0).ToString();
                    ...
                    RenderQuizInfo += "<span>";
                    RenderQuizInfo += item.QuizTitle.Length > 15 ? item.QuizTitle.Substring(0, 15) + ".." : item.QuizTitle;
        ...
        public ActionResult QuizProcced(QuizStartInfo objInfo)
        {
            ...
            if (list != null)
            {
                ...
                {
                    html += "<span><a href='#' class='QuizBack' my-data='" + Crypto.Encrypt(objInfo.QuestionID) + "," + Crypto.Encrypt(objInfo.QuizID) + "'><span class='mdi mdi-chevron-left'></span>Prev</a></span>";
                }
                if (list.IsPauseAllowed)
                {
                    html += "<span><a href='#' class='QuizBack btn btn-outline-info quiz-buttons' my-data='" + encryptQuestionID + "," + encryptQuizID + "'><i class='mdi mdi-pause-circle-outline'></i></a></span>";
                }
                else
                {
                    html += "<span style='display:none'><a href='#' class='QuizPause' my-data='" + encryptQuestionID + "," + encryptQuizID + "'><i class='mdi mdi-pause-circle-outline'></i></a></span>";
        ...
        {
            int tempPercentage = int.Parse(Percentage);
