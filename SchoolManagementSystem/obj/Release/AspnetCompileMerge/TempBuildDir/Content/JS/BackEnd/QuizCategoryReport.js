...
var rowNumber = 0;
function ParamToLoadQuizCategoryReportList(e) {
    var grid = $("#QuizCategoryReport").data("kendoGrid").dataSource;
    var objInfo1 =
   {
       CategoryID: ($('#ddlSearchQuizCategory').val() === '') ? -1 : $('#ddlSearchQuizCategory').val(),
       QuizTitle: $('#txtSearchQuizTitle').val(),
...
function LoadCategoryGrid() {
    $("#grid").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: {
                    url: "/Admin/QuizCategoryReport/GetAllQuiz",
        ...
        columns: [
          // { title: "&nbsp;", template: "#= ++record #", width: 30 ,hidden},
            { field: "CategoryName", title: "CategoryName" },
             { field: "QuizTitle", name: "QuizTitle", title: "Quiz" },
...
function renderRecordNumber(data) {
    var page = parseInt($("#QuizCategoryReport").data("kendoGrid").dataSource.page()) - 1;
    var pagesize = $("#QuizCategoryReport").data("kendoGrid").dataSource.pageSize();
...
function RenderSearch_KendoParamater(SearchDataObj) {
    ...
    parameter.CategoryID = SearchDataObj.CategoryID;
    parameter.QuizTitle = SearchDataObj.QuizTitle;
