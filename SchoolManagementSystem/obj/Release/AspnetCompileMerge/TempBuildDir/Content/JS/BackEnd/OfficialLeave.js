...
var id, employeeid, organisationid, isconnected;
$(document).ready(function () {
    ...
    $("#takeLeaveSubmit").off().on('click', function (e) {
        ...
        if (!$('form#formTakeLeave').data('unobtrusiveValidation').validate()) {
            ...
            $.ajax({
                url: "/Admin/OfficialLeave/SaveOfficialLeave",
                type: 'POST',
...
function ParamToOfficialLeaveList(e) {
    var grid = $("#OfficialLeaveListGrid").data("kendoGrid").dataSource;
    return {
...
function resetRowNumberOfficialLeave(e) {
    ...
    //    var grid = $("#OfficialLeaveListGrid").data("kendoGrid");
    //    var row = this;
    ...
    //    }
    //})
    $("#OfficialLeaveListGrid tbody tr .k-grid-Approve").each(function () {
        var currentDataItem = $("#OfficialLeaveListGrid").data("kendoGrid").dataItem($(this).closest("tr"));
...
function Details(e) {
    ...
    if (!isNepaliDate) {
        ...
        $("#dateto").text(new Date(dataItem.NepaliDateTo).toLocaleDateString());
    }
...
{    
    $.ajax({
        url: "/Admin/OfficialLeave/ApproveLeave",
        type: 'POST',
        data: {
