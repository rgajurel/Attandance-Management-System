...
$(document).ready(function () {
    ...
    $(document).on("change", "#btnUpload", function () {
        debugger;
        //oFileIn = document.getElementById('btnUpload');
        //if (oFileIn.addEventListener) {
        //    oFileIn.addEventListener('change', ReadExcelFile, false);
        //}

        ReadExcelFile($('#btnUpload').prop('files'));
     //Do something
});

    $("#marksSaveRecords").hide();
    ...
    $("#calculatecase").off().on('click', function (e)
    {
        ...
        else
        {
            ...
            for (i = 0; i < dataItem.length; i++) 
            {
                ...
                if (firstItem["ObtainedMarksTheory"] != 0)
                {
                    datacount++;
                }
            ...
            if(datacount==0)
            {
                ShowMessage("Please first Enter Marks");
            }
            else
            {
                var dataItem;
                ...
                $("#marksSaveRecords").click();                
                $('#MarksEntryCase').modal('show');
                $("#downloadcase").click();
            }
        }
    ...
    $("#downloadcase").off().on('click', function (e) {
      
        debugger;
        $('#ExcelMarksEntryCase').empty();
        dataItem = $("#MarksEntryGrid").data("kendoGrid").dataSource.data();
       ...
       html+='<thead>'
        ...
        //html += '<th>ClassID</th>';
        //html += '<th>Section</th>';
        //html += '<th>FacultyID</th>';
        //html += '<th>SubjectID</th>';
        //html += '<th>TermID</th>';
        //html += '<th>SessionID</th>';
        html += '<th>StudentID</th>';
        html += '<th>StudentName</th>';
        $(subsubject).each(function (i, result)
        {
            if (result.ClassID == $("#ClassID").val())
            {
                html += '<th>' + result.SubSubjectName + '</th>';
            }
        ...
        {           
            debugger;
            Item = $('#MarksEntryGrid').data().kendoGrid.dataSource.data()[i];
            if (Item["IsAdmin"] == true)
            {
                html += '<tr>';
                //html += '<td>' + $("#ClassID").val() + '</td>';
                //html += '<td>' + $("#Section").val() + '</td>';
                //html += '<td>' + $("#FacultyID").val() + '</td>';
                //html += '<td>' + $("#SubjectID").val() + '</td>';
                //html += '<td>' + $("#TermID").val() + '</td>';
                //html += '<td>' + $("#SessionID").val() + '</td>';
        ...
        $('table#ExcelMarksEntryCase').append(html);
...
function GetSubSubject() {
    $.ajax({
        url: "/Admin/GradeMaster/GetAllSubSubject",
        type: "POST",
        dataType: "json",
          global:false,      
        success: function (output1)
        {
            ...
            $(output1).each(function (i, result) {
                subsubject.push(result);
            });
   ...
   for (i = 0, f = files[i]; i != files.length; ++i)
    {
        ...
        reader.onload = function (e) {
            ...
            workbook.SheetNames.forEach(function (sheetName) {
                ...
                if (roa.length > 0) {
                    ChequeImport.config.ExcelDataObject = roa;
                    var data = JSON.stringify(roa).replace(/ /g, "");
                    SendExcelObject(data);
                    console.log(data);
                }
            });
        };
...
function SendExcelObject(data) {
    ...
    $.ajax({
        url: "/Users/CreateBatchUpload/",
        data: '{ "LstExcelObject":' + data + '}',
        type: "POST",
        dataType: "json",
        ...
        success: function (result) {
            ...
            location.reload();
            // window.location = "/Admin/ScanCheque/Message?message=" + result;
